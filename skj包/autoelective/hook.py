...
from .utils import json_dump, xSHA1
...
def _dump_request(r):
    ...
    filename = "%s.%s.json" % (timestamp, basename)  # put timestamp first
    file = os.path.normpath(os.path.abspath(os.path.join(_USER_REQUEST_LOG_DIR, filename)))

    # Full request objects include cookies, credentials and executable pickle
    # state. Persist only the metadata needed to correlate a parser failure.
    snapshot = {
        "captured_at": timestamp,
        "method": r.request.method,
        "url": redact_sensitive(r.url),
        "status_code": r.status_code,
        "content_type": r.headers.get("Content-Type"),
        "response_bytes": len(r.content),
        "response_sha1": xSHA1(r.content),
    }
    json_dump(snapshot, file, ensure_ascii=False, indent=2)

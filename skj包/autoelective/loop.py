...
def _get_refresh_interval(consecutive_failures=0):
    if refresh_random_deviation <= 0:
        base = max(4.0, refresh_interval)
    else:
        delta = (random.random() * 2 - 1) * refresh_random_deviation * refresh_interval
        base = max(4.0, refresh_interval + delta)
    if consecutive_failures <= 0:
        return base
    backoff = min(300.0, 4.0 * (2 ** min(consecutive_failures - 1, 7)))
...
def run_iaaa_loop():
    ...
    environ.add_event("info", "IAAA 登录线程已启动")
    ...
    while True:
        ...
        try:
            ...
            environ.add_event("success", "选课会话 %s 登录成功" % elective.id)
            cout.info("")
            ...
            if login_succeeded:
                ...
                t = max(login_loop_interval, min(120.0, 2 ** min(consecutive_failures, 7)))
            cout.info("")
...
def run_elective_loop():
    ...
    noWait = False
    consecutive_failures = 0
    environ.phase = "starting"
    environ.add_event("info", "选课线程已启动")
    ...
    while True:
        ...
        try:
            ...
            if supply_cancel_page == 1:
                ...
                environ.last_poll_at = int(time.time())
                environ.phase = "running"
                ...
                try:
                    ...
                    if config.is_debug_dump_request:
                        ...
                        cout.info("Raw response dump skipped because debug_dump_request is disabled")
            ...
            while len(tasks) > 0:
                ...
                try:
                    ...
                    environ.add_event("success", "%s 补选请求已成功" % course.name)
            ...
            ferr.warning(e)
            _add_error(e)
            environ.phase = "waiting_window"
            environ.operation_enabled = False
            environ.add_event("warning", "当前不在补退选时段，网络轮询已自动停止")
            cout.warning("Not in operation time; stop elective network requests")
            reloginPool.put_nowait(killedElective)
            noWait = True
            return
            ...
            if noWait:
                ...
                if poll_succeeded:
                    ...
                    consecutive_failures += 1
                    environ.phase = "backoff"
                    environ.add_event(
                        "warning",
                        "连续请求失败 %d 次，已进入退避等待" % consecutive_failures,
                    )
                t = _get_refresh_interval(consecutive_failures)
                environ.next_poll_at = int(time.time() + t)
                cout.info("")
                cout.info("======== END Loop %d ========" % environ.elective_loop)

...
项目默认仅启动本地准备页面，不登录、不请求选课网：

```console
$ python3 main.py
```

然后访问 `http://127.0.0.1:7074`。可以在页面中整理课程优先级、互斥组、名额阈值与请求间隔，草稿只保存在本机浏览器。导出的安全配置不包含学号和密码。

只有在官方补退选时段内、修复所有配置问题后，才能显式开启网络请求：

```console
$ python3 main.py --enable-election --with-monitor
```

正常轮询按用户设置的正数间隔执行，请求串行；网络异常保留退避等待。如果服务器返回“不在操作时段”，网络轮询会自动停止。

> 登录信息仍由本地 `config.ini` 读取。请勿上传、分享或打包这个文件。验证码使用项目内的 ONNX 模型在本地识别，不调用云端 OCR。

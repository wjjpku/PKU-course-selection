...
let html=fs.readFileSync(path.join(root,'app/web/index.html'),'utf8');
...
let app=fs.readFileSync(path.join(root,'app/web/app.js'),'utf8');
for(const [a,b] of [
...
for(const name of ['styles.css','planner.css','course-query.js','course-planner.js','workbench-insights.js'])files['assets/'+name]=fs.readFileSync(path.join(root,'app/web',name),'utf8');

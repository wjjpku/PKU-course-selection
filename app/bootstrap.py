...
student_id = os.environ.get("PKU_STUDENT_ID", "")
password = os.environ.get("PKU_PASSWORD", "")
is_dual_degree = True #是否双学位账号
...
def predict_captcha(im_data, model):
    code = model.recognize(im_data).code
    return code


def main():
    if not student_id or not password:
        raise RuntimeError("请先通过 PKU_STUDENT_ID 和 PKU_PASSWORD 环境变量提供登录信息")
    model = CaptchaRecognizer(CNN_MODEL_FILE, CAPTCHA_LABELS_FILE)

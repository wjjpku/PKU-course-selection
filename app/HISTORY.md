...
v7.0.0 (2026-09-05)
-------------------
- 新增本地准备期工作台，支持课程排序、互斥组、名额阈值、频率校验和安全配置导出
- 默认禁止选课网络请求，必须通过 `--enable-election` 明确开启
- 非补退选时段自动停止，并将实际请求间隔限制为不低于 4 秒
- 监控服务改为本机监听、关闭调试模式，状态接口不返回登录信息
- 日志自动遮蔽 token、学号、Cookie 和密码，请求快照不再使用 pickle
- 修复 NumPy 2 兼容、体育课异常继承和异常捕获顺序问题

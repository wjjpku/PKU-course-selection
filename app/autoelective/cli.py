...
def create_default_parser():
    ...
    parser.add_option(
        '--enable-election',
        dest='enable_election',
        action='store_true',
        default=False,
        help='explicitly enable elective network requests (disabled by default)',
    )
...
def setup_default_environ(options, args, environ):
    ...
    environ.operation_enabled = options.enable_election
    environ.phase = "starting" if options.enable_election else "preparation"
...
def create_default_threads(options, args, environ):
    ...
    # Import after parse_args() so a custom config is visible to module singletons.
    if options.enable_election:
    ...
    # Preparation mode always starts the local dashboard. During an election it
    # remains opt-in for backwards compatibility with headless deployments.
    if options.with_monitor or not options.enable_election:
...
def run():
    ...
    if options.enable_election:
        ...
        errors = AutoElectiveConfig().election_validation_errors()
        if errors:
            parser.error("选课网络请求未启动，请先修复配置：\n- " + "\n- ".join(errors))
